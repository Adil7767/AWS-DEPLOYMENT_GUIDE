# Complete AWS Ubuntu Deployment Guide for api.bridgebond.io
...
PORT=2323
HOST=0.0.0.0
BASE_URL=https://api.bridgebond.io
...
EMAIL_FROM=noreply@bridgebond.io
...
### Step 9.2: Create Nginx Configuration for api.bridgebond.io

```bash
# Create Nginx configuration file
sudo nano /etc/nginx/sites-available/api.bridgebond.io
```
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/api.bridgebond.io/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.bridgebond.io/privkey.pem;
    ...
    # Logging
    access_log /var/log/nginx/api.bridgebond.io-access.log;
    error_log /var/log/nginx/api.bridgebond.io-error.log warn;
...
sudo ln -s /etc/nginx/sites-available/api.bridgebond.io /etc/nginx/sites-enabled/
...
3. **Verify DNS is working:**
   ...
   ping api.bridgebond.io
   
   # Or use dig
   dig api.bridgebond.io
   
   # Or use nslookup
   nslookup api.bridgebond.io
   ```
...
sudo certbot --nginx -d api.bridgebond.io
...
Certificate is saved at: /etc/letsencrypt/live/api.bridgebond.io/fullchain.pem
Key is saved at: /etc/letsencrypt/live/api.bridgebond.io/privkey.pem
```
...
curl -I https://api.bridgebond.io
```
...
Enter `api.bridgebond.io` and check if it resolves to your server IP globally.
...
# 7. Test API endpoint
curl https://api.bridgebond.io/v1/docs
```

### Step 13.2: Test from Browser

1. Open your browser
2. Visit: `https://api.bridgebond.io`
3. Visit: `https://api.bridgebond.io/v1/docs` (API documentation)
...
sudo tail -f /var/log/nginx/api.bridgebond.io-access.log

# View Nginx error logs
sudo tail -f /var/log/nginx/api.bridgebond.io-error.log
...
sudo tail -50 /var/log/nginx/api.bridgebond.io-error.log
...
2. Add/update A record: `api.bridgebond.io → YOUR_SERVER_IP`
...
ls -la /etc/nginx/sites-available/api.bridgebond.io
ls -la /etc/nginx/sites-enabled/api.bridgebond.io
...
openssl s_client -connect api.bridgebond.io:443 -servername api.bridgebond.io
```
...
sudo certbot certonly --standalone -d api.bridgebond.io
sudo systemctl start nginx
...
sudo certbot --nginx -d api.bridgebond.io --staging
```
...
- [ ] Can reach server from outside: `curl http://api.bridgebond.io`
- [ ] Domain resolves correctly: `dig api.bridgebond.io`
...
### Issue 5: Domain Mismatch (api.bridgebond.ai vs api.bridgebond.io)
...
- [ ] DNS A record configured (api.bridgebond.io → server IP)
- [ ] SSL certificate obtained and configured
- [ ] Firewall (UFW) configured
- [ ] Application accessible via https://api.bridgebond.io
- [ ] API documentation accessible at https://api.bridgebond.io/v1/docs
- [ ] Backups configured
- [ ] Monitoring set up
